...
import { useAddTypeMutation } from "../../generated/graphql";
...
function AddType() {
  ...
  async function onSubmit(values: any) {
    ...
    const { data } = await addType({ args });
    if (data?.addType.error) {
      console.log("The error cam eback: ", data.addType.error.message);
      return seterror(data?.addType.error.message);
    } else if (!data?.addType.error && values.addCategories)
      return history.push({
        pathname: "/admin/add-category",
        state: {
          type: data?.addType.data,
        },
      });
    else
      return toast({
